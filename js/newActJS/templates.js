...
const Helper = (() => {
    ...
    const playAudio = async ({ playBtn = '.common_playBtn', pauseBtn = '.common_pauseBtn', handleBtn = true } = {}) => {
        ...
        try {
            ...
            $('#question_header_container, #question-container-box').show();
            if (handleBtn) {
                $(playBtn).hide();
                $(pauseBtn).show();
            }
    ...
    const setAudio = (src) => {
        return new Promise((resolve, reject) => {
            ...
            __audio.addEventListener(
            'loadedmetadata',
            () => resolve(__audio.duration),
            { once: true }
        );
    ...
    const translatePopupLabels = (lang = 'hi') => {
        if (lang == 'hi') {
            return {
                ...
                attemptAll: "कृपया सभी प्रश्नों का प्रयास करें"
            };
        } else {
            return {
                excellent: 'Excellent!',
                ...
                attemptAll: "Please attempt all questions"
            };
        }
    };
...
const MatchLeftToRight = (() => {
    ...
    const showAnswers = (activityId) => {
        try {
            ...
            for (const leftId in correctMatches) {
                ...
                leftItem.classList.add("correct", 'noClick');
                rightItem.classList.add("correct", 'noClick');
    ...
    const resetActivity = (activityId) => {
        try {
            ...
            if (!area) return;
            area.querySelectorAll(".item").forEach(item => {
                item.classList.remove("correct", "wrong", "selected", "noClick");
            });
    ...
    const resetActivity = (activityId) => {
        try {
            ...
            qSA(".matchItems1, .imgBoxes, .matchItems2, .centerItems, .matchItems3, .imgBoxes", container)
                .forEach(el => el.classList.remove("correct", "wrong", "selected", "noClick"));
    ...
    const showAnswers = (activityId) => {
        try {
            ...
            Object.entries(activity.correctLeft || {}).forEach(([leftId, centerId]) => {
                ...
                drawLine(svg, leftEl, centerEl, "green", "L");
                leftEl.classList.add("correct", 'noClick');
                centerEl.classList.add("correct", 'noClick');
            ...
            Object.entries(activity.correctRight || {}).forEach(([rightId, centerId]) => {
                ...
                drawLine(svg, rightEl, centerEl, "green", "R");
                rightEl.classList.add("correct", 'noClick');
                centerEl.classList.add("correct", 'noClick');
...
const MatchTopToBottom = (() => {
    ...
    const resetActivity = (activityId) => {
        try {
            ...
            area.querySelectorAll(".item2").forEach(it => it.classList.remove("selected", "correct", "wrong", "showAnsMatch3", "noClick"));
            Activity.toggleCheckBtn('.submit-btn', false);
        } catch (err) {
    ...
    const showAnswers = (activityId) => {
        try {
            ...
            for (const topId in correctMatches) {
                ...
                drawArrow(activityId, topItem, bottomItem, "green");
                topItem.classList.add("correct", "showAnsMatch3", "noClick");
                bottomItem.classList.add("correct", "showAnsMatch3", "noClick");
            ...
            <div class="question">
                                    <div class="container">
                                        ...
                                        <div id='question-container-box' ${audioSrc ? `style="display: none"` : ''}>
                                            ...
                                            ${definedCol instanceof Object && Object.entries(definedCol).length ? 
    ...
    const fillInTheBlanks = (questionId) => {
        try {
            ...
            content?.questions.forEach((question, qIndex) => {
                ...
                if (colCondition) div.classList.add(`row`, `flex-wrap`, `col-sm-${col.sm}`, `col-md-${col.md}`, `col-${col.col}`, `${subQuestion.length == 0 && !inputBelowCondition ? 'align-items-center' : 'align-items-start'}`);
                ...
                if (subQuestion.length || inputBelowCondition) {
                    ...
                    html.push(`<div class='d-flex flex-wrap align-items-start'>${imageView}</div>`);
                }
                ...
                if (subQuestion.length) {
                    ...
                    subQuestion.map((subques, subind) => {
                        ...
                        const bullet = subQuestion.length > 1
                            ...
                            : '';
                        const final = `<div class="my-2 d-flex flex-wrap">
                                            ${ subQueLabel ? `<div style="width: 30px">${subQueLabel ? `${bullet}` : ''}</div>` :''}
                                            <div class='col'>
                                                ${subQuesText.join('')}
                                                ${multiInput.join('')}
                                            </div>
                    ...
                    if (inputBelowCondition) {
                        html.push(`<div class='${question?.question ? 'mt-2': ''}'>`);
                        question?.answers.map((_, i) => {
                            const belowInput = `<div class="mb-2">
                                ${renderInput({
                                qID: questionId,
                                sqID: false,
                                inputIndex: i,
        ...
        try {
            ...
            jumbleData.forEach((word, index) => {
                ...
                word.split('').forEach(letter => {
                    letterRow.append(`<div class="letterjumb correct noClick">${letter}</div>`);
                });
            });
        } catch (err) {
...
const Mcq_PathKaSaar = (() => {
    ...
    const ui = (questionId) => {
        try {
            ...
            const only_text = !content.img && (!content.mcq || content.mcq.length === 0);

            const moralArray = [ 'moral', 'नैतिक शिक्षा' ];
...
const DragAndDrop = (() => {
    ...
    const ui = (questionId) => {
        try {
                                ...
                                <div class="container">
                                    <div class="rowWithAudios">
                                        ...
                                        <div class="playsBtns">
                                            ...
                                            <svg fill="currentColor" id="pauseSvg" class="bi bi-pause btnSounds common_pauseBtn" viewBox="0 0 16 16">
            ...
            const src = activity?.content?.audio;
            Helper.setAudio(Activity.pathToCWD() + src);
...
const DragAndDropMulti = (() => {
    ...
    const showPopUp = () => {
        ...
        const tableBodyF = `<div class="table-responsive p-2">
                            <table class="table table-bordered" style="font-size:20px">
                                <thead class="text-light" style="white-space: nowrap;">
                                    <tr>
                                        ${__getQuestionData().isBullets ? `<th>${headLabels.sequence}</th>` : '' }
                                        <th>${headLabels.attempted}</th>
                                        <th>${headLabels.correct}</th>
                                        <th>${headLabels.result}</th>
                                    </tr>
        ...
        if (hasTypeSet) {
            ...
            const compareAnswerArrays = (correctAns = [], userAns = []) => {
                ...
                userAns.map((ans, key) => {
                    ...
                    const body = `
                        <tr clsss='trData'>
                            ${__getQuestionData().isBullets ? `<th>(${Activity.translateBulletLabels({ lang: lang, key: key })})</th>` : '' }
            ...
            shuffledQuestions.forEach((item, i) => {
                ...
                const body = `
                    <tr clsss='trData'>
                        ${__getQuestionData().isBullets ? `<th>(${Activity.translateBulletLabels({ lang: lang, ind: i })})</th>` : '' }
                        <td class="${isCorrect ? 'text-success' : 'text-danger'}">
                ...
                const html = `
                    <div class="my-2 p-1">
                        <div class="row g-0 border rounded h-100 p-2 ">
                            ${ __getQuestionData().isBullets ? `
                                    <div class="col-auto me-1 d-flex align-items-start">
                                        (${Activity.translateBulletLabels({ lang: lang, ind: ind })})
                                    </div>
                                ` : ''
                            }
                ...
                const html = `
                    ${col_size && !singleQuestionMode ? `
                        ...
                        <div class="d-flex h-100 border rounded p-2">
                            ${__getQuestionData().isBullets ? `
                                    <div class="col-auto p-1 d-flex align-items-start me-1">
                                        (${Activity.translateBulletLabels({ lang: lang, ind: ind })})
                                    </div>
                                ` : ''
                            }
    ...
    const renderDataDND = (questionId) => {
        try {
            ...
            const questionHtml = [];
            if (hasTypeSet) {
                const questionData = __getQuestionData();
    ...
    const __getQuestionData = () => {
        ...
        const isBullets = questions.length > 1;
    ...
    const __showAnswerPopupSingleType = () => {
        ...
        const tablePartStart = `
            ...
            <table class="table table-bordered" style="font-size:20px">
                <thead class="text-light" style="white-space: nowrap;">
                    <tr>
                        ${__getQuestionData().isBullets ? `<th>${tableHeadLabel.sequence}</th>` : '' }
...
const Pdf = (() => {
    ...
    const ui = (questionId) => {
        try {
            ...
            const tabBtns = Define.get('buttons')?.length;
...
const Shrutlekh = (() => {
    ...
    const ui = (questionId) => {
        try {
            ...
            const src = audioBundle['clickBtn']?.[lang];
            Helper.setAudio(src);
            const audio_playBtns = parent.querySelectorAll('.common_playBtn');
            if(audio_playBtns){
                audio_playBtns.forEach(btn => {
    ...
    const toggleNextWord = () => {
        ...
        if (questionIndex != questions.length) {
            const src = audioBundle['clickNextBtn']?.[lang];
            Helper.setAudio(src);
            Helper.playAudio();
            renderWordButton();
        }
    ...
    const playAudio_focusInput = () => {
        try {
            ...
            const src = Activity.pathToCWD() + curQues.audio;
            Helper.setAudio(src);
    ...
    const checkPracAnswer = async () => {
        try {
            ...
            if ($('.correction-input').length == correctCount && correctCount > 1) {
                ...
                const src = audioBundle['correct']?.[lang];
                await Helper.setAudio(src);
                Helper.playAudio();
                const timeout = Math.round(Helper.audio.duration * 1000);
                ...
                const src = audioBundle[boxID]?.[lang];
                await Helper.setAudio(src);
                Helper.playAudio();
                const timeout = Math.round(Helper.audio.duration * 1000);
                const popupLabels = Activity.translatePopupLabels(lang);
    ...
    const wrongPopUp = async () => {
        ...
        const src = audioBundle['incorrect']?.[lang];
        await Helper.setAudio(src);
        Helper.playAudio();
    ...
    const renderCorrectionBox = async () => {
        ...
        const src = audioBundle['writeCorrectBelow']?.[lang];
        Helper.setAudio(src);
        Helper.playAudio();
...
const WordSearch = (() => {
    ...
    const isFalse = (value='') => ['', null, false, undefined].includes(value);
    ...
    const ui = (questionId) => {
        try {
            ...
            const mainHeadingText     = activity?.config?.questionSection?.heading?.main?.text;            
            const mainHeadingTextBool = !isFalse(mainHeadingText) ?? mainHeadingText;
                            ...
                            <div class="onTheImagesFill2">
                                <div class="question word-searched">                                    
                                    <div class="container" id="${containerId}">
                                        <div class="${Helper.vars.head}"></div>
                                        <div class="hints" style="${mainHeadingTextBool !== false ? 'display:block;' : 'display:none;'}">
                                            ${mainHeadingText}
                                        </div>
    ...
    const render = (questionId) => {
        try {
            ...
            if (queSection != undefined) {
                ...
                if (
                    ques?.across.length 
                    && !isFalse(queHeading?.vertical?.text)
                ) puzzle.push(`<div class="hints">${queHeading?.vertical?.text ?? 'Across'}</div>`);
                ...
                if (
                    ques?.down.length 
                    && !isFalse(queHeading?.horizontal?.text)
                ) puzzle.push(`<div class="hints">${queHeading?.horizontal?.text ?? 'Down'}</div>`);
...
const TextArea = (() => {
    ...
    const render = (questionId) => {
        try {
            ...
            shuffledQuestions = isShuffled 
                                    ? Activity.shuffleArray(content?.questions ?? []) 
                                    : (content?.questions ?? []);
            // ..
    ...
    const resetAnswers = () => {
        ...
        if (input) input.forEach(input => {
            input.value = ''
            autoResizeTextarea(input);
            input.classList.remove('noClick');
        });
        $('.submit-btn').removeClass('disable');
    };
...
const RachnatmakWithInputs = (() => {
                                    ...
                                    <div class="container">
                                        <div class="wrapers">
                                            ...
                                            `<div class="row p-1" style="font-size: 20px;">
                                                <div class="fw-bold col-md-${colLeft?.md} col-sm-${colLeft?.sm} col-${colLeft?.col}">${content?.heading?.left ?`<div class='border-bottom'>${content?.heading?.left}</div>`: ''}</div>
                                                <div class="fw-bold col-md-${colRight?.md} col-sm-${colRight?.sm} col-${colRight?.col}">${content?.heading?.right ?`<div class='border-bottom'>${content?.heading?.right}</div>`: ''}</div>
                                            </div>`:''}
                                            <div id="showInputsAnnds" style="${lang == 'hi' ? 'display: none;' : ''}"></div>
                                            ${content?.showButtons ?
                                                `<div class="buttons machiNgs">
    ...
    const renderActivity = (questionId) => {
        ...
        document.querySelectorAll('.fillAppli').forEach(el => {
            if (!el.value.trim()) {
                el.value = '';
            }
        });
    ...
    const resetActivity = () => {
        const activity = Activity.getDefine(Activity.getQid(`#${containerId}`)) ?? {};
        const content = activity?.content ?? {};
        const showAnswerOfId = content?.showAnswerOfId;
        ...
        textareas.forEach((ta) => {
            const match = ta.id.match(/_(\d+)$/);

            if (!match) return;

            const rowIndex = Number(match[1]);
            const keepIndex = Number(showAnswerOfId) - 1;

            if (rowIndex === keepIndex) {
...
const ClickOnImage = (() => {
    ...
    const resetActivity = () => {
        $(".submit-btn").removeClass("noClicked");
        document.querySelectorAll('.imgClick').forEach(card => {
            card.classList.remove('selectedClickImgs', 'correct-borderClickImgs', 'wrong-borderClickImgs', 'noClick');
        });
    }
    ...
    const showAnswers = (dataKeyParam, btn) => {
        ...
        data.forEach((item, ind) => {
            ...
            spans.forEach((span) => {
                ...
                if (answers.includes(span.dataset.word) && span.dataset.word.trim() != span.innerHTML.trim()) {
                    span.classList.add("circle");
                }
            });
                    ...
                    ? `
                        <div class="question-text-container row g-0 gap-2  fs-5 align-items-start">
                            ${title?.main?.text && title.main.text != ''
                                ? `
                                    <div class="col-lg-4 col-md-4 col-sm-12 col-12 fw-bold text-light text-uppercase rounded-3">
                                        <p class="frintBoxDesignVT2">${title.main.text}</p>
                                    </div>
                                ` : ''
                    }
        ...
        container.innerHTML = questions.map((q, qIndex) => {
            ...
            return `
                <div class="question-block mb-3 p-2 border-bottom row g-0 align-items-center">
                    ${ queLen != 1 ? `<div class="col-auto w50 me-1">${bullet}.</div>`: ''}
                ...
                <tbody>
                    ${results.map(r => `
                        <tr>
                            ...
                            <td>
                                <span 
                                    class="cu-tag ${r.chosen} ${r.chosen !== 'none'
